import { signIn } from '@/auth';

export default function LandingPage() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 p-4 dark:bg-zinc-950">
      <div className="w-full max-w-4xl space-y-12">
        {/* Hero Section */}
        <div className="text-center">
          <h1 className="mb-4 text-5xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50 sm:text-6xl">
            Pick Your Battles
          </h1>
          <p className="mx-auto max-w-2xl text-xl text-zinc-600 dark:text-zinc-400">
            Focus on what matters. Limit yourself to a few tasks at a time — 3 by default, customizable to fit your workflow.
          </p>
        </div>

        {/* Value Proposition */}
        <div className="rounded-lg border border-zinc-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          <h2 className="mb-4 text-2xl font-bold text-zinc-900 dark:text-zinc-50">
            Why Limit Your Tasks?
          </h2>
          <p className="mb-6 text-zinc-600 dark:text-zinc-400">
            Overwhelming task lists lead to paralysis and procrastination. By forcing yourself to
            choose only a few tasks at a time (3 by default), you&apos;re compelled to ruthlessly prioritize what truly
            matters. Complete a task to make room for the next one. Your limit is yours to set.
          </p>
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="text-center">
              <div className="mb-2 text-3xl">🎯</div>
              <h3 className="mb-1 font-semibold text-zinc-900 dark:text-zinc-50">
                Forced Focus
              </h3>
              <p className="text-sm text-zinc-600 dark:text-zinc-400">
                No more endless to-do lists. Just what matters now.
              </p>
            </div>
            <div className="text-center">
              <div className="mb-2 text-3xl">✅</div>
              <h3 className="mb-1 font-semibold text-zinc-900 dark:text-zinc-50">
                Real Progress
              </h3>
              <p className="text-sm text-zinc-600 dark:text-zinc-400">
                See tasks completed, not a growing backlog.
              </p>
            </div>
            <div className="text-center">
              <div className="mb-2 text-3xl">🧘</div>
              <h3 className="mb-1 font-semibold text-zinc-900 dark:text-zinc-50">
                Less Stress
              </h3>
              <p className="text-sm text-zinc-600 dark:text-zinc-400">
                Achieve more by doing less, intentionally.
              </p>
            </div>
          </div>
        </div>

        {/* How It Works */}
        <div className="rounded-lg border border-zinc-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          <h2 className="mb-6 text-2xl font-bold text-zinc-900 dark:text-zinc-50">
            How It Works
          </h2>
          <div className="space-y-6">
            <div className="flex gap-4">
              <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-zinc-900 text-lg font-bold text-white dark:bg-zinc-50 dark:text-zinc-900">
                1
              </div>
              <div>
                <h3 className="mb-1 font-semibold text-zinc-900 dark:text-zinc-50">
                  Sign in with Google
                </h3>
                <p className="text-sm text-zinc-600 dark:text-zinc-400">
                  Quick and secure authentication. Your tasks are private and synced across devices.
                </p>
              </div>
            </div>
            <div className="flex gap-4">
              <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-zinc-900 text-lg font-bold text-white dark:bg-zinc-50 dark:text-zinc-900">
                2
              </div>
              <div>
                <h3 className="mb-1 font-semibold text-zinc-900 dark:text-zinc-50">
                  Choose your battles
                </h3>
                <p className="text-sm text-zinc-600 dark:text-zinc-400">
                  Add tasks up to your configured limit (default: 3). The system won&apos;t let you add more until you complete one.
                </p>
              </div>
            </div>
            <div className="flex gap-4">
              <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-zinc-900 text-lg font-bold text-white dark:bg-zinc-50 dark:text-zinc-900">
                3
              </div>
              <div>
                <h3 className="mb-1 font-semibold text-zinc-900 dark:text-zinc-50">
                  Stay focused and complete
                </h3>
                <p className="text-sm text-zinc-600 dark:text-zinc-400">
                  Work on your priorities without distraction. Complete tasks to make room for new ones.
                </p>
              </div>
            </div>
          </div>
        </div>

        {/* Call to Action */}
        <div className="text-center">
          <form
            action={async () => {
              'use server';
              await signIn('google', { redirectTo: '/' });
            }}
          >
            <button
              type="submit"
              className="inline-flex cursor-pointer items-center gap-3 rounded-lg bg-zinc-900 px-8 py-4 text-lg font-medium text-white transition-colors hover:bg-zinc-800 dark:bg-zinc-50 dark:text-zinc-900 dark:hover:bg-zinc-200"
            >
              <svg className="h-6 w-6" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                  fill="#4285F4"
                />
                <path
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  fill="#34A853"
                />
                <path
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  fill="#FBBC05"
                />
                <path
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  fill="#EA4335"
                />
              </svg>
              Sign in with Google
            </button>
          </form>
          <p className="mt-4 text-xs text-zinc-500 dark:text-zinc-500">
            Free to use. We only use your Google account for authentication.
            <br />
            We never access your Gmail or other Google services.
          </p>
        </div>
      </div>
    </div>
  );
}
