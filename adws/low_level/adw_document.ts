...
async function main() {
  ...
  if (argv.length < 2) {
    console.error('Usage: ts-node adw_document.ts <issue-number> <adw-id>');
    process.exit(1);
  }
  const issueNumber = argv[0];
  const adwIdArg = argv[1];
  const logger = setupLogger(adwIdArg, 'adw_document');
  ...
  await makeIssueComment(issueNumber, formatIssueMessage(adwId, 'ops', '✅ Starting documentation generation'));

  const req = { agent_name: 'ops', slash_command: '/document', args: [planFile], adw_id: adwId } as any;
  const resp = executeTemplate(req);
  if (!resp.success) {
    logger.error(`Documentation failed: ${resp.output}`);
    await makeIssueComment(issueNumber, formatIssueMessage(adwId, 'docs', `❌ Documentation failed: ${resp.output}`));
    process.exit(1);
  }
  await makeIssueComment(issueNumber, formatIssueMessage(adwId, 'docs', `✅ Documentation completed: ${resp.output}`));
  state.save('adw_document');
}

main().catch((e) => { console.error('Fatal error in adw_document:', e); process.exit(1); });
